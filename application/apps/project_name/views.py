...
ITEMS_PER_PAGE = getattr(settings, "{{ project_name|upper }}_ITEMS_PER_PAGE", 25)
...
def article_list(request, template_name='{{ project_name }}/article_list.html'):
...
@login_required
@permission_required('{{ project_name }}.add_article')
def add_article(
        request, form_class=forms.ArticleForm, extra_context=None,
        template_name='{{ project_name }}/article_form.html'):
    ...
    if form.is_valid():
        ...
        return redirect('{{ project_name }}-article-detail', form.instance.pk)
    context_dict.update(form=form)
    return render(request, template_name, context_dict)


@login_required
@permission_required('{{ project_name }}.change_article')
def change_article(
    ...
    if form.is_valid():
        ...
        return redirect(request.GET.get(REDIRECT_FIELD_NAME, reverse(
            '{{ project_name }}-article-detail', args=[slug])))
    context_dict.update(form=form, object=obj, cut_marker='<!--more-->')
    return render(request, template_name, context_dict)


@login_required
@permission_required('{{ project_name }}.delete_article')
def delete_article(
        request, pk, extra_context=None,
        template_name='{{ project_name }}/article_confirm_delete'):
